"""
AI-Agent-Claude Lambda

Handles all Claude/Anthropic agent operations:
- Invoke: Send messages to Claude with personas, skills, and context
- Sessions: Manage conversation history
- Personas/Skills: Loaded from DynamoDB

Environment Variables:
- ANTHROPIC_API_KEY: Claude API key (or use Secrets Manager)
...
# Anthropic SDK - vendored in Lambda package
import anthropic
...
def get_api_key() -> str:
    """Get Anthropic API key from environment or Secrets Manager."""
    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if api_key:
        return api_key

    # Try Secrets Manager
    secret_name = os.environ.get("ANTHROPIC_SECRET_NAME", "ai-agent/anthropic-api-key")
    try:
...
def build_system_prompt(persona: Dict, context: Optional[Dict] = None) -> str:
...
def skills_to_tools(skills: List[Dict]) -> List[Dict]:
    """Convert skill definitions to Claude tool format."""
    tools = []
    for skill in skills:
        tool = {
...
def invoke_claude(
    ...
    """
    Invoke Claude with persona, skills, and context.
    ...
    Args:
        ...
        stream: Whether to stream response (not supported in Lambda)
    ...
    model = persona.get("model", "claude-sonnet-4-20250514")
    ...
    if skills:
        request_params["tools"] = skills_to_tools(skills)
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    Request format:
    {
        ...
        "message": "Draft an outreach email",
        "context": {
            "user": {"name": "John Doe", ...}
        }
    }
